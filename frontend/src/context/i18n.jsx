import React, { createContext, useContext, useEffect, useMemo, useState } from 'react';const LanguageContext = createContext({ lang: 'en', t: (k) => k, setLang: () => {} });const DICT = {
  ...
  es: {
    ...
    leftYouStart: 'Izquierda (T� empiezas)',
    ...
    homeConfirmText: 'Est�s a punto de salir del juego. �Est�s seguro?',
    cancel: 'Cancelar',
    leave: 'Salir',
    save: 'Guardar',
    close: 'Cerrar',
    helpTitle: 'Ayuda',
    helpText: 'Selecciona una ficha o la pelota; haz clic en las casillas resaltadas para mover o patear. Hay pateo forzado cuando est�s adyacente y la pelota no es neutral.',
    configTitle: 'Configuraci�n del Juego',
    landingTagline: 'Un juego de estrategia que combina la emoci�n del f�tbol con decisiones t�cticas profundas y posicionamiento inteligente.',
    pitchGrid: 'Rejilla del Campo',
    difficultyLevels: 'Niveles de Dificultad',
    realTime: 'En tiempo real',
    turnFeedback: 'Indicador de Turno',
    learnMoreThesis: 'Conoce la tesis y el contexto de investigaci�n',
    ruleBook: 'Libro de Reglas',
    ruleObjectiveBanner: 'Objetivo: �Anota goles para ganar!',
    ruleDetails: 'Detalles',
    ruleBasicsLabel: 'B�sicos',
    ...
    ruleBasicsItems: [
      'El campo es de 15 filas � 11 columnas. La franja central contiene las porter�as.',
    ...
    ruleMoveTitle: 'C�mo Mover',
    ruleMoveLead: 'Selecciona una ficha y mu�vela a una casilla v�lida adyacente.',
    ruleMoveItems: [
      'Los destinos legales se resaltan; las bandas exteriores est�n fuera de juego.',
      'No puedes entrar en casillas exteriores m�s all� del borde crema.',
      'Al caer sobre la pelota tomas posesi�n salvo si aplica pateo forzado.'
    ],
    ruleKickLabel: 'Pateo',
    ruleKickTitle: 'Patear y Pasar',
    ruleKickLead: 'Si tu ficha tiene la pelota, patea a cualquier casilla alcanzable resaltada.',
    ruleKickItems: [
      'Los tiros siguen l�neas rectas hasta chocar; el servidor valida la legalidad.',
      'Puede haber pateo forzado al estar adyacente y la pelota no ser neutral.',
      'Crea �ngulos para abrir l�nea de disparo hacia el arco central.'
    ],
    ruleScoreLabel: 'Anotar',
    ruleScoreTitle: 'C�mo Anotar',
    ...
    ruleScoreItems: [
      ...
      'Tras un gol, el juego se reinicia seg�n el torneo.'
    ],
    configureMatch: 'Configurar una Partida',
    aboutTitle: 'Acerca de esta Tesis',
    researchContext: 'Contexto de Investigaci�n',
    researchBlurb: 'Este proyecto eval�a agentes de IA competitivos en un entorno de f�tbol por turnos (Mastergoal), comparando Minimax, MCTS y estrategias heur�sticas en distintos tama�os de equipo. La web permite reproducir condiciones de prueba y visualizar el comportamiento de los agentes.',
    credits: 'Cr�ditos',
